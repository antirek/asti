  ...
  io.on('connection', function (socket) {
    ...
    client.on('call', function (data) {
      ...
      call.originate(data, client)
        .then(function (res) {          
          client.emitToSocket('call', {actionid: actionid, data: res, status: 'ok'});
        })
        .fail(function (err) {
          client.emitToSocket('call', {actionid: actionid, data: err, status: 'error'});
        });
    });
  });
};
